...
// Quick helper function that takes an arbitrary number of version strings and checks if any of them match OSVER
function IsOsVersion() {
	for (var i = 0; i < arguments.length; i++) {
		...
		if (OSVER.indexOf(arguments[i] + ".") === 0) {
			return true
		}
	}
...
function getMP3Length(folder, file) {
	...
	var lengthArray = folder.GetDetailsOf(file, LENGTH_INDEX).split(":").reverse()
	var length = 0
...
for (var i = 0; i <= 5; i++) {
	var part = fso.OpenTextFile(FULL_SCRIPT_PARTS_DIR + "\\music_part" + i + ".lua", ForReading, false, TriStateFalse).ReadAll()
	musicLua.Write(part)
...
var MUSIC_FOOTER = fso.OpenTextFile(FULL_SCRIPT_PARTS_DIR + "\\music_footer.lua", ForReading, false, TriStateFalse).ReadAll()
