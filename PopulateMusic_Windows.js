...
if (typeof WScript === "undefined") {
  throw new Error(
    "Unsupported environment. Make sure you're running this script with Microsoft(R) Windows Based Script Host."
  );
}
...
// Quick helper function that takes an arbitrary number of version strings and checks if any of them match OS_VER
/** @param {...string} args */
function IsOsVersion() {
  for (var i = 0; i < arguments.length; i++) {
    ...
    if (OS_VER.indexOf(arguments[i] + ".") === 0) {
      return true;
    }
  }
...
if (IsOsVersion("10.0", "6.3", "6.2", "6.1", "6.0")) {
...
var FULL_SCRIPT_PARTS_DIR = PBM_DIR + "\\ScriptParts";
...
function getMP3Length(folder, file) {
  // We reverse the array so the seconds are first, the minutes second, etc.
  var lengthArray = folder
    .GetDetailsOf(file, LENGTH_INDEX)
    .split(":")
    .reverse();
  ...
  for (var ind = 0; ind <= lengthArray.length; ind++) {
    if (typeof lengthArray[ind] != "undefined") {
      // Always specify radix otherwise numbers starting with 0 will be treated as octal
      var num = parseInt(lengthArray[ind], 10);
...
// JScript doesn't include any sort of sprintf (C) or string.format (Lua) function (or access to .NET, which does have one),
// so we have to use a whole lot of concatenation.
...
function addFiles(path) {
  ...
  for (var i = 0; i < files.Count; i++) {
    ...
    if (extension) {
      ...
      if (invalidLength)
        WScript.Echo("Warning: " + path + " has an invalid or zero length!");
...
for (var i = 0; i <= 5; i++) {
  var part = fso
    .OpenTextFile(
      FULL_SCRIPT_PARTS_DIR + "\\music_part" + i + ".lua",
      ForReading,
      false,
      TriStateFalse
    )
...
var MUSIC_FOOTER = fso
  .OpenTextFile(
    FULL_SCRIPT_PARTS_DIR + "\\music_footer.lua",
    ForReading,
    false,
    TriStateFalse
  )
...
WScript.Echo(
  "Added " + addedTotal + " music files to music.lua (" + addedStr + ")"
);
